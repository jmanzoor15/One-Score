<template>
  <div class="bg-white w-[300px] ml-4 mt-4 border-[1px] rounded-md">
    <div class="flex flex-col items center p-2" v-for="news in data.data" :key="news.uuid">
      <a :href="`/football/news/${news.uuid}`">
        <div class=" flex">
          ...
          <div class="w-[158px] h-[79px] ml-2">
            <div class=" h-[65px]  overflow-hidden">
              <span class="text-[12px] font-bold text-gray-500 bottom-2 relative">{{ truncateText(news.title) }}</span>
            </div>
